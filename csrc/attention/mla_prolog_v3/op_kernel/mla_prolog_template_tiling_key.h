 ...
 * \brief Trimmed for vllm-ascend:
 *   CACHE_MODE=PA_BSND(1)
 *   QUANT_MODE=0 (BF16 non-quant) / 7 (MXFP8 kv no-quant) / 8 (MXFP8 kv per-tensor)
 */
...
// -------------------------- Mxfp8全量化 + kv pertensor量化 + PA_BSND --------------------------
#if ORIG_DTYPE_TOKEN_X == -1 || ORIG_DTYPE_WEIGHT_UQ_QR == -1 || ORIG_DTYPE_KV_CACHE == -1 || \
